...
// Agent semantic page operations run in a named isolated world rather than the
// page's default world. WebKitGTK isolated worlds share the page's DOM and
// resources but are not subject to the page's Content Security Policy, so the
// semantic recorder (snapshot/interact/inspect/diagnostics) keeps working on
// pages that set a strict `script-src 'self'` CSP. The recorder is injected in
// the same world so diagnostics state is visible to the reader.
constexpr const char *agent_world_name = "vantage-agent";
...
constexpr const char *dark_mode_bootstrap = R"JS((()=>{
if(window.__vantageDarkReaderApplied)return;
...
const alreadyDark=()=>{const roots=[document.documentElement,document.body].filter(Boolean);let score=0;for(const e of roots){const s=getComputedStyle(e),b=parse(s.backgroundColor),c=parse(s.color);if(b&&b[3]>.2&&lum(b)<.24)score+=2;if(c&&lum(c)>.55)score++}return score>=3};
if(alreadyDark()){window.__vantageDarkReaderApplied='native-dark';return;}
if(!window.DarkReader){console.warn('Vantage Dark Mode: Dark Reader API did not load');return;}
try{DarkReader.setFetchMethod(window.fetch);DarkReader.enable({brightness:100,contrast:100,sepia:0,darkSchemeBackgroundColor:'#0b0b0c',darkSchemeTextColor:'#e8e6e3'});window.__vantageDarkReaderApplied='darkreader';}
catch(error){console.error('Vantage Dark Mode: Dark Reader failed',error);}
})())JS";

std::string dark_reader_source() {
    static const std::string source = [] {
        std::vector<std::filesystem::path> candidates;
        std::error_code error;
        const auto executable = std::filesystem::read_symlink("/proc/self/exe", error);
        if (!error) candidates.push_back(executable.parent_path().parent_path() / "assets" / "darkreader.js");
        candidates.push_back(std::filesystem::current_path() / "assets" / "darkreader.js");
...
struct ApplicationState {
    ...
    vantage::AgentEventLog agent_events{2048};
...
void load_changed(WebKitWebView *view, WebKitLoadEvent event, TabState *tab) {
    tab->window->owner->agent_events.publish("page.load", "{\"tab_id\":" + std::to_string(tab->id) + ",\"event\":" + std::to_string(static_cast<int>(event)) + "}");
        ...
        if (vantage::dark_mode_enabled_for_domain(domain))
            {
                ...
                if (!dark_reader.empty()) {
                    const auto script = dark_reader + "\n" + dark_mode_bootstrap;
                    webkit_web_view_evaluate_javascript(view, script.c_str(), script.size(), nullptr, nullptr, nullptr, nullptr, nullptr);
                } else {
                    g_warning("Dark Mode is enabled but assets/darkreader.js is unavailable");
                }
            }
    }
...
void web_process_terminated(WebKitWebView *, WebKitWebProcessTerminationReason reason, TabState *tab) {
    ...
    tab->window->owner->agent_events.publish("page.web_process_terminated", "{\"tab_id\":" + std::to_string(tab->id) + ",\"reason\":" + std::to_string(static_cast<int>(reason)) + "}");
}
...
gboolean load_failed(WebKitWebView *view, WebKitLoadEvent, const char *failing_uri,
    ...
    tab->window->owner->agent_events.publish("page.load_failed", "{\"tab_id\":" + std::to_string(tab->id) + ",\"message\":" + vantage::json_string(tab->last_load_error) + "}");
...
TabState *new_tab(WindowState *state, const std::string &uri, bool load_initial) {
    ...
    auto *agent_script = webkit_user_script_new_for_world(agent_diagnostics_script,
        WEBKIT_USER_CONTENT_INJECT_ALL_FRAMES, WEBKIT_USER_SCRIPT_INJECT_AT_DOCUMENT_START, agent_world_name, nullptr, nullptr);
...
struct AgentCookiesResult { std::string id; vantage::AgentReply reply; };
...
void agent_wait_finished(GObject *source,GAsyncResult *result,void *raw){std::unique_ptr<AgentWaitState> state(static_cast<AgentWaitState*>(raw));GError*error=nullptr;auto*value=webkit_web_view_evaluate_javascript_finish(WEBKIT_WEB_VIEW(source),result,&error);if(error){state->reply(vantage::agent_error(state->id,"wait_error",error->message));g_error_free(error);if(value)g_object_unref(value);return;}const bool ready=value&&jsc_value_is_boolean(value)&&jsc_value_to_boolean(value);if(value)g_object_unref(value);if(ready){state->reply(vantage::agent_ok(state->id,"true"));return;}if(g_get_monotonic_time()>=state->deadline){state->reply(vantage::agent_error(state->id,"timeout","wait condition not met"));return;}auto*next=state.release();g_timeout_add(50,[](void*p)->gboolean{agent_wait_check(static_cast<AgentWaitState*>(p));return G_SOURCE_REMOVE;},next);}
void agent_wait_check(AgentWaitState *state){webkit_web_view_evaluate_javascript(state->view,state->script.c_str(),-1,agent_world_name,"vantage-agent://page.wait",nullptr,agent_wait_finished,state);}
...
gboolean dispatch_agent_request(void *raw) {
    ...
    if (r.method == "downloads.list") {std::string out="[";bool first=true;for(const auto&e:owner->data->downloads()){if(!first)out+=",";first=false;out+="{\"id\":"+std::to_string(e.id)+",\"uri\":"+vantage::json_string(e.uri)+",\"destination\":"+vantage::json_string(e.destination)+",\"status\":"+vantage::json_string(e.status)+",\"received\":"+std::to_string(e.received)+",\"total\":"+std::to_string(e.total)+"}";}out+="]";done(vantage::agent_ok(r.id,out));return G_SOURCE_REMOVE;}
    if (r.method == "downloads.cancel" || r.method == "downloads.remove") {auto id=vantage::json_param_integer(r.params_json,"id");if(r.method=="downloads.cancel")cancel_download(owner,id);else owner->data->remove_download(id);done(vantage::agent_ok(r.id,"true"));return G_SOURCE_REMOVE;}
    if (r.method == "bookmarks.list") {std::string out="[";bool first=true;for(const auto&e:owner->data->bookmarks()){if(!first)out+=",";first=false;out+="{\"uri\":"+vantage::json_string(e.uri)+",\"title\":"+vantage::json_string(e.title)+"}";}out+="]";done(vantage::agent_ok(r.id,out));return G_SOURCE_REMOVE;}
    if (r.method == "bookmarks.add" || r.method == "bookmarks.remove") {auto uri=vantage::json_param_string(r.params_json,"uri");if(r.method=="bookmarks.add")owner->data->add_bookmark({uri,vantage::json_param_string(r.params_json,"title")});else owner->data->remove_bookmark(uri);done(vantage::agent_ok(r.id,"true"));return G_SOURCE_REMOVE;}
    if (r.method == "history.list") {std::string out="[";bool first=true;for(const auto&e:owner->data->history()){if(!first)out+=",";first=false;out+="{\"id\":"+std::to_string(e.id)+",\"uri\":"+vantage::json_string(e.uri)+",\"title\":"+vantage::json_string(e.title)+",\"visited_at\":"+std::to_string(e.visited_at)+"}";}out+="]";done(vantage::agent_ok(r.id,out));return G_SOURCE_REMOVE;}
    if (r.method == "history.clear") {owner->data->clear_history();done(vantage::agent_ok(r.id,"true"));return G_SOURCE_REMOVE;}
    if (r.method == "permissions.list") {std::string out="[";bool first=true;for(const auto&e:owner->data->permissions()){if(!first)out+=",";first=false;const char*d=e.decision==vantage::Permission::allow?"allow":e.decision==vantage::Permission::deny?"deny":"ask";out+="{\"origin\":"+vantage::json_string(e.origin)+",\"capability\":"+vantage::json_string(e.capability)+",\"decision\":"+vantage::json_string(d)+"}";}out+="]";done(vantage::agent_ok(r.id,out));return G_SOURCE_REMOVE;}
    ...
    if (r.method == "page.inspect") {
        auto*t=agent_tab(owner,static_cast<vantage::TabId>(vantage::json_param_integer(r.params_json,"tab_id")));if(!t){done(vantage::agent_error(r.id,"not_found","tab not found"));return G_SOURCE_REMOVE;}auto kind=vantage::json_param_string(r.params_json,"kind");std::string script;if(kind=="text")script="document.body?document.body.innerText:''";else if(kind=="html")script="document.documentElement?document.documentElement.outerHTML:''";else if(kind=="selection")script="String(getSelection?getSelection():'')";else if(kind=="metadata")script="({url:location.href,title:document.title,viewport:{width:innerWidth,height:innerHeight},scroll:{x:scrollX,y:scrollY},document:{width:document.documentElement.scrollWidth,height:document.documentElement.scrollHeight}})";else{done(vantage::agent_error(r.id,"invalid_params","kind must be text, html, selection, or metadata"));return G_SOURCE_REMOVE;}const std::string wrapped="(()=>{const __v=eval("+javascript_string(script)+");const __j=JSON.stringify(__v);return __j===undefined?'null':__j})()";webkit_web_view_evaluate_javascript(t->view,wrapped.c_str(),-1,agent_world_name,"vantage-agent://page.inspect",nullptr,agent_javascript_finished,new AgentJavascriptResult{r.id,std::move(done)});return G_SOURCE_REMOVE;
    }
    if (r.method == "page.interact") {
    ...
    if (r.method == "page.snapshot") {
        ...
        const std::string wrapped="(()=>{const __v=eval("+javascript_string(script)+");return JSON.stringify(__v)})()"; webkit_web_view_evaluate_javascript(t->view,wrapped.c_str(),-1,agent_world_name,"vantage-agent://page.snapshot",nullptr,agent_javascript_finished,new AgentJavascriptResult{r.id,std::move(done)});return G_SOURCE_REMOVE;
    }
    if (r.method == "events.since" || r.method == "events.clear") {
        if(r.method=="events.clear"){owner->agent_events.clear();done(vantage::agent_ok(r.id,"true"));return G_SOURCE_REMOVE;}
        const auto after=static_cast<std::uint64_t>(std::max<long long>(0,vantage::json_param_integer(r.params_json,"after",0)));
        const auto limit=static_cast<std::size_t>(std::clamp<long long>(vantage::json_param_integer(r.params_json,"limit",256),1,1024));
        auto events=owner->agent_events.since(after,limit);std::string out="{\"latest\":"+std::to_string(owner->agent_events.latest())+",\"dropped\":"+std::to_string(owner->agent_events.dropped())+",\"events\":[";bool first=true;for(const auto&e:events){if(!first)out+=",";first=false;out+="{\"sequence\":"+std::to_string(e.sequence)+",\"type\":"+vantage::json_string(e.type)+",\"data\":"+e.payload_json+"}";}out+="]}";done(vantage::agent_ok(r.id,out));return G_SOURCE_REMOVE;
    }
    if (r.method == "page.diagnostics" || r.method == "page.diagnostics.clear") {
        ...
        webkit_web_view_evaluate_javascript(t->view,wrapped.c_str(),-1,agent_world_name,"vantage-agent://page.diagnostics",nullptr,agent_javascript_finished,new AgentJavascriptResult{r.id,std::move(done)});return G_SOURCE_REMOVE;
    }
