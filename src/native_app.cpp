...
std::string new_tab_page(WindowState *state) {
    ...
    return "<!doctype html><html><head><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'>"
        ...
        ".suggestions{display:none;border:1px solid #4a4844;border-top:0;border-radius:0 0 10px 10px;background:#2b2a29;padding:5px 6px 7px;box-shadow:0 4px 10px #0005}.suggestions.open{display:block}"
        ".item{display:flex;align-items:center;gap:9px;min-width:0;padding:7px 11px;border-radius:6px;color:#eee9df;cursor:default}.item.active{background:#45433f;outline:1px solid #74b928;outline-offset:-1px}.item .query{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.item small{flex:none;color:#aaa59c}.item small:before{content:'– ';}.searchIcon{position:relative;width:14px;height:14px;flex:none}.searchIcon:before{content:'';position:absolute;left:1px;top:1px;width:7px;height:7px;border:1.5px solid #aaa59c;border-radius:50%}.searchIcon:after{content:'';position:absolute;left:9px;top:9px;width:5px;height:1.5px;background:#aaa59c;transform:rotate(45deg);transform-origin:left center}"
        ...
        "function paint(){[...panel.children].forEach((e,i)=>e.classList.toggle('active',i===active))}function showList(){const q=input.value.trim().toLowerCase();shown=history.filter(v=>!q||v.toLowerCase().includes(q)).slice(0,10);panel.replaceChildren(...shown.map((v,i)=>{const d=document.createElement('div');d.className='item';d.innerHTML='<span class=searchIcon></span><span class=query></span><small>Google Search</small>';d.querySelector('.query').textContent=v;d.onpointermove=()=>{active=i;paint()};d.onmousedown=e=>e.preventDefault();d.onclick=()=>choose(i);return d}));if(shown.length){panel.classList.add('open');input.classList.add('open')}else closeList();paint()}"
