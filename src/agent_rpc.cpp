...
int run_agent_cli(const std::vector<std::string_view>&a){if(a.empty()){std::cerr<<"usage: vant agent <status|version|capabilities|call|js|events|watch> ...\n";return 2;}std::string method(a[0]),params="{}";if(method=="events"||method=="watch")method="events.since";if(method=="js"){std::string script;if(a.size()>=3&&a[1]=="--file"){std::ifstream in{std::string(a[2])};script.assign(std::istreambuf_iterator<char>(in),{});}else if(a.size()>=2&&a[1]!="-")script=std::string(a[1]);else script.assign(std::istreambuf_iterator<char>(std::cin),{});method="page.javascript";params="{\"script\":"+json_string(script)+"}";}else if(method=="call"){if(a.size()<2){std::cerr<<"vant agent call: method required\n";return 2;}method=std::string(a[1]);if(a.size()>2)params=std::string(a[2]);}int fd=::socket(AF_UNIX,SOCK_STREAM,0);sockaddr_un u{};u.sun_family=AF_UNIX;auto path=agent_socket_path();std::strcpy(u.sun_path,path.c_str());if(fd<0||::connect(fd,reinterpret_cast<sockaddr*>(&u),sizeof u)!=0){std::cerr<<"vant agent: Vantage agent socket unavailable at "<<path<<"\n";if(fd>=0)::close(fd);return 1;}auto q=request_json("cli",method,params);if(!write_all(fd,q)){::close(fd);return 1;}std::string out;char b[4096];ssize_t n;while((n=::recv(fd,b,sizeof b,0))>0)out.append(b,static_cast<std::size_t>(n));::close(fd);std::cout<<out;return out.find("\"ok\":true")!=std::string::npos?0:1;}
}
